        ...
        try {
            ...
            if (typeJson == null) {
                throw new JsonSyntaxException(
                    "Achievement \"" + id + "\" is missing required property \"condition/type\"!");
            }
            AchievementProperties props = AchievementProperties.parse(json, id);

            // condition must be parsed last
            AmazingTrophiesAPI.getAchievementConditionHandler(typeJson.getAsString())
                .parse(id, conditionJson);

...
public class AmazingTrophiesAPI {
     ...
     * @param handlerSupplier A {@link Supplier} which provides a new instance of the ConditionHandler to register each time {@link Supplier#get()} is called. All instances provided by this Supplier must have the same functionality and {@link ConditionHandler#getID() id}.
     * @author glowredman
     */
    public static void registerConditionHandler(@Nonnull Supplier<ConditionHandler> handlerSupplier) {
        ...
        if(ACHIEVEMENT_CONDITION_HANDLERS.containsKey(id)) {
            LOGGER.error("Condition Handler with id {} already exists! {} will not be registered.", id, achievementHandler.getClass().getName());
            return;
        }
    ...
    public static void registerAchievements() {
        ACHIEVEMENTS.values().forEach(AchievementProperties::register);
    }
    
    private static Path getConfigDir() {
        try {
            Path path = Launch.minecraftHome.toPath().resolve("config").resolve("amazingtrophies");
